...
class PneumoniaDatasetInMem(Dataset):
    def __init__(
            self,
            root,
            dataidxs=None,
...
class PneumoniaDataset(Dataset):
    ...
    def __build_dataset__(self, dataset=None):
        if dataset is None:
            data = []
            target = []
            for f in glob.glob(os.path.join(self.root, "**", "*.jpg"), recursive=True):
...
def load_pneumonia_data(datadir):
    train_transform, val_transform = get_transforms()

    train_dataset = PneumoniaDataset(root=os.path.join(datadir, "train"), transform=train_transform)
    test_dataset = PneumoniaDataset(root=os.path.join(datadir, "test"), transform=val_transform)
...
def partition_data(dataset, datadir, partition, n_nets, alpha):
    ...
    X_train, y_train, X_test, y_test = load_pneumonia_data(datadir)
    n_train = len(X_train)
    # n_test = X_test.shape[0]

    ix_map_path = f"pneumonia_dataidx_map_{n_nets}.pickle"
    dist_info_path = f"pneumonia_dist_{n_nets}.pickle"
...
def get_dataloader(datadir, train_bs, test_bs,
    ...
    train_dl = DataLoader(dataset=train_ds, batch_size=train_bs, shuffle=True, pin_memory=True, num_workers=4)
    test_dl = DataLoader(dataset=test_ds, batch_size=test_bs, shuffle=False, num_workers=2)

    return train_dl, test_dl


def load_partition_data_pneumonia(
        dataset,
        data_dir,
        partition_method,
        partition_alpha,
    ...
    data_dir = os.path.join(data_dir, "chexpert_pneumonia")
