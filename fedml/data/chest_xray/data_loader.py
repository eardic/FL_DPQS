...
class PneumoniaDatasetFiles(Dataset):
    def __init__(
            self,
            root,
            dataidxs=None,
...
def partition_data(dataset, datadir, partition, n_nets, alpha):
    ...
    ix_map_path = f"chestxray_dataidx_map_{n_nets}.pickle"
    dist_info_path = f"chestxray_dist_{n_nets}.pickle"
...
def get_dataloader(datadir, train_bs, test_bs,
    ...
    train_dl = DataLoader(dataset=train_ds, batch_size=train_bs, shuffle=True, pin_memory=True)
    test_dl = DataLoader(dataset=test_ds, batch_size=test_bs, shuffle=False, num_workers=2)

    return train_dl, test_dl


def load_partition_data_chest_xray(
        dataset,
        data_dir,
        partition_method,
        partition_alpha,
