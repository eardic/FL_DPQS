...
class CheXpert(data.Dataset):
    ...
    def __init__(
            self, data_dir, label_dir=None, dataidxs=None, dataset=None, train=True, transform=None, download=False,
            policy="zeros"
    ):
        """
        Args:
        ...
        self.images, self.labels = self._build_datasets(dataset)
    ...
    def _build_datasets(self, dataset=None):
        images = []
        labels = []
        if dataset is None:
