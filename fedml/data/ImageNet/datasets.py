...
class ImageNet32Noise(object):
    def __init__(self, file, transform: Optional[Callable] = None, img_size=32, n_classes=10) -> None:
        super().__init__()
        self.root = file
        self.transform = transform
        ...
        with open(file, 'rb') as fo:
            ...
            self.labels = np.array(data["labels"], copy=False)
            self.size = len(self.labels)
            img_size2 = img_size * img_size
            self.image = np.dstack((self.image[:, :img_size2],
                                    self.image[:, img_size2:2 * img_size2],
                                    self.image[:, 2 * img_size2:]))
            self.image = self.image.reshape((self.image.shape[0], img_size, img_size, 3))

        exc_file_path = os.path.join(os.path.split(file)[0], "cls_exclusion")
        if os.path.exists(exc_file_path):
            ...
            with open(exc_file_path, "r") as f:
                for line in f:
                    parts = line.split("-")
                    if len(parts) == 1:
                        exc_list.append(int(line.strip()))
                    elif len(parts) == 2:
                        exc_list.extend(list(range(int(parts[0]), int(parts[1]) + 1)))
                print(f"Class exclusion length: {len(exc_list)} !")
            filter = [l not in exc_list for l in self.labels]
            self.labels = self.labels[filter]
            self.image = self.image[filter]
            self.size = len(self.labels)
        else:
            print("ImageNet class exclusion file not found :", exc_file_path)
