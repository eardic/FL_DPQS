...
class BreakHisV1Dataset(Dataset):
    def __init__(
            self,
            root,
            dataidxs=None,
...
def get_transforms(image_size=128):
    ...
    train_transform = transforms.Compose([
        ...
        transforms.RandomApply([transforms.RandomRotation(degrees=15, interpolation=InterpolationMode.BICUBIC)],
                               p=0.25),
...
def load_breakhisv1_data(datadir):
    train_transform, val_transform = get_transforms()

    train_dataset = BreakHisV1Dataset(root=os.path.join(datadir, "train"), transform=train_transform)
    test_dataset = BreakHisV1Dataset(root=os.path.join(datadir, "test"), transform=val_transform)
...
def partition_data(dataset, datadir, partition, n_nets, alpha):
    ...
    X_train, y_train, X_test, y_test = load_breakhisv1_data(datadir)
    n_train = len(X_train)
    # n_test = X_test.shape[0]

    ix_map_path = f"breakhisv1_dataidx_map_{n_nets}.pickle"
    dist_info_path = f"breakhisv1_dist_{n_nets}.pickle"
...
def load_partition_data_breakhisv1(
        dataset,
        data_dir,
        partition_method,
        partition_alpha,
    ...
    data_dir = os.path.join(data_dir, "breakhis_v1")
