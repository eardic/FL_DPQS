...
class PansmearDataset(Dataset):
    ...
    def __build_dataset__(self, dataset=None):
        if dataset is None:
            data = []
            target = []
            for f in glob.glob(os.path.join(self.root, "**", "*.bmp")):
...
def load_pansmear_data(datadir):
    train_transform, val_transform = get_transforms()

    train_dataset = PansmearDataset(root=os.path.join(datadir, "train"), transform=train_transform)
    test_dataset = PansmearDataset(root=os.path.join(datadir, "test"), transform=val_transform)
...
def partition_data(dataset, datadir, partition, n_nets, alpha):
    ...
    X_train, y_train, X_test, y_test = load_pansmear_data(datadir)
    n_train = len(X_train)
    # n_test = X_test.shape[0]

    ix_map_path = f"pansmear_dataidx_map_{n_nets}.pickle"
    dist_info_path = f"pansmear_dist_{n_nets}.pickle"
...
def load_partition_data_pansmear(
        dataset,
        data_dir,
        partition_method,
        partition_alpha,
    ...
    data_dir = os.path.join(data_dir, "pansmear")
